...
versions=$(\
    ssh $user@$backup_host ls /data/backup/ | \
    sed -r 's/^nextcloud-([0-9]*)$/\1/g' | grep -v .sql\
)
...
ssh $user@$backup_host "echo $back_sudo_pass | sudo -S -u nextsavior /home/nextsavior/restore.sh $chosen_version"
